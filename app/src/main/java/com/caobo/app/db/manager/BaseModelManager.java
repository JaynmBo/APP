...
//            model.setCurrentUser(currentUser);
     ...
     * @param context
     * @param nPage                第几页
     * @param nModelCountOfPerPage 每页多少条数
     * @param attributeName        属性名称
     * @param attribute            属性
     * @param columnName           排序的字段名称
        ...
        try {
            ...
            if (count > 0) {
                ...
                builder.orderBy(columnName, ascending)
                        .offset(nPage * nModelCountOfPerPage).limit(count)
    ...
    public <T extends BaseModel> List<T> getPageModels(Class<T> clazz,
        ...
        try {
            ...
            if (count > 0) {
                ...
                builder.orderBy(columnName, ascending)
                        .offset(nPage * nModelCountOfPerPage).limit(count)
                        .where().eq(attributeName, attribute);
                list = baseModelDao.query(builder.prepare());
            }
        } catch (SQLException e) {
            // TODO Auto-generated catch block
    ...
    @SuppressWarnings({"rawtypes", "deprecation"})
    public <T extends BaseModel> List<T> getPageModels(Class<T> clazz,
                                                       Class dbHelperClass, Context context, String attributeName, String attribute,
                                                       String columnName, boolean ascending) {
        List<T> list = new ArrayList<T>();
        try {
            Dao<T, String> baseModelDao = getDao(clazz, context, dbHelperClass);
            QueryBuilder<T, String> builder = baseModelDao.queryBuilder();
            builder.orderBy(columnName, ascending).where().eq(attributeName, attribute);
            list = baseModelDao.query(builder.prepare());
        } catch (SQLException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
...
//			builder.where().eq("currentUser", currentUser);
    ...
    public FileBean  getModelsByName(Context context, String attribute) {
        if (TextUtils.isEmpty(attribute)) {
            return null;
        }
        FileBean fileBean = null;
        List<FileBean> fileBeans = getModelsByAttribute(FileBean.class, DBOpenHelper.class, context,
                "url", attribute);
     ...
     */
    @SuppressWarnings("rawtypes")
    public <T extends BaseModel> List<T> getModelsByAttributeNoUser(Class<T> clazz,
                                                                    Class dbHelperClass, Context context, String columnName, String attribute) {
        List<T> models = new ArrayList<T>();
        try {
            Dao<T, String> baseModelDao = getDao(clazz, context, dbHelperClass);
            QueryBuilder<T, String> builder = baseModelDao.queryBuilder();
            builder.where().eq(columnName, attribute);
            models = baseModelDao.query(builder.prepare());
            System.out.println("ghj");
        } catch (SQLException e) {
            // TODO Auto-generated catch block
    ...
    public <T extends BaseModel> void delModels(Class<T> clazz, Class dbHelperClass,
        ...
        try {
            ...
            builder.where().eq(columnName, attribute);
//                    .and().eq("currentUser", currentUser);
